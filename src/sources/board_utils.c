...
int check_tile_data(int position, char type, int target, int board_size) {
    if ((position < 0) || (position >= board_size)) return ERROR;
    if ((target < 0) || (target >= board_size)) return ERROR;
    if ((type != 'S') && (type != 'L')) return ERROR;
    if ((type == 'S') && (target >= position)) return ERROR;
    if ((type == 'L') && (target <= position)) return ERROR;
    return SUCCESS;
}
...
int read_tile_line(Board* board, FILE* fd) {
    int position, target, matches;
    char type;
    matches = fscanf(fd, "%d %c %d", &position, &type, &target);
    if (matches != 3) return INVALID_TILE_LINE;
    if (ERROR == check_tile_data(position, type, target, get_size(board))) return INVALID_TILE_DATA;
    Tile* tile = get_tile_at(board, position);
    tile -> target_position = target;
    return SUCCESS;
}
...
int read_board_file(Board* board, FILE* fd) {
    int rows, columns, matches, ret_code;
    char x;
    matches = fscanf(fd, "%d %c %d", &rows, &x, &columns);
    if (matches != 3) return INVALID_BOARD_DIMENSIONS;
    if((rows < 1) || (rows > MAX_ROWS) || (columns < 1) || (columns > MAX_COLUMNS)) return INVALID_BOARD_DIMENSIONS;

    init_board(board, rows, columns);

    while ((!feof(fd))) {    //determina si ha arribat al final del fitxer o no
        ret_code = read_tile_line(board, fd);
 ...
 * @return
 *
 * Pre: The variable Board has to be defined and the variables row, column and tile has to be defined
...
int load_board(Board* board, char* path) {
    ...
    if (fd == NULL){
        ...
        int ret_code = read_board_file(board, fd);
        fclose(fd);
        return ret_code;
    }
}